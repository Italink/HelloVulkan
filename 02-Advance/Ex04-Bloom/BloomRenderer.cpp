...
vk::ImageView BloomRenderer::getOutputImageView()
{
	return frameBuffer_[0].imageView;
}

void BloomRenderer::init(vk::Device device)
{
	this->device = device;
...
void BloomRenderer::render(vk::Image image, int width, int height, vk::CommandBuffer cmdBuffer)
{
	if (!isInitialized()) {
		return;
	}
	vk::Image currentImage = image;

	if (frameBufferFormathasInit) {
		...
		cmdBuffer.pipelineBarrier(vk::PipelineStageFlagBits::eHost, vk::PipelineStageFlagBits::eTransfer, {}, {}, {}, barrier);
		frameBufferFormathasInit = false;
...
void BloomRenderer::destroyFrameBuffer()
{
...
bool BloomRenderer::isInitialized()
{
	return frameBuffer_[0].framebuffer != 0;
}

void BloomRenderer::updateFrameBuffer(int width, int height, uint32_t memoryIndex)
{
	frameBufferFormathasInit = true;
	if (isInitialized()) {
		destroyFrameBuffer();
	}
	...
	vk::MemoryAllocateInfo memAllocInfo(memReq.size, memoryIndex);
