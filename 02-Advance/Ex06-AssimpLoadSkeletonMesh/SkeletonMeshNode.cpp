...
SkeletonMeshNode::SkeletonMeshNode(SkeletonMesh* model, const aiMesh* mesh, const aiScene* scene, aiMatrix4x4 matrix)
	: model_(model)
	, localMatrix_(matrix)
...
void SkeletonMeshNode::initVulkanResource(vk::Device device, int memoryTypeIndex)
{
	device_ = device;
